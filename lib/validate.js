...
function teamsData(value) {
  return arr(value, 'teamsData', { max: 30 }).map((t, i) => {
    ...
    // fixos pode ser array de strings (legado) OU array de {nome,peso}.
    const fixosNorm = arr(t.fixos, `teamsData[${i}].fixos`, { max: 30 }).map((f, j) => {
      if (typeof f === 'string') {
        return { nome: str(f, `teamsData[${i}].fixos[${j}]`, { max: 80 }), peso: 1 };
      }
      if (!f || typeof f !== 'object') throw bad(`teamsData[${i}].fixos[${j}] deve ser texto ou objeto`);
      return {
        nome: str(f.nome, `teamsData[${i}].fixos[${j}].nome`, { max: 80 }),
        peso: Number.isFinite(Number(f.peso)) && Number(f.peso) > 0 ? Number(f.peso) : 1,
      };
    });
    let vagasExtras = 2;
    if (t.vagasExtras !== undefined && t.vagasExtras !== null && t.vagasExtras !== '') {
      const n = Number(t.vagasExtras);
      if (!Number.isFinite(n) || n < 0 || n > 20) {
        throw bad(`teamsData[${i}].vagasExtras inválido (0-20)`);
      }
      vagasExtras = n;
    }
    return {
