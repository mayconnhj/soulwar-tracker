...
export async function addQuest(rawPayload) {
  ...
  const questRow = {
    ...
    bonecos_pilotados: payload.bonecosPilotados || []
  };
...
export async function updateQuest(id, rawPayload) {
  ...
  if (payload.ausentes !== undefined) questRow.ausentes = payload.ausentes;
  if (payload.bonecosPilotados !== undefined) questRow.bonecos_pilotados = payload.bonecosPilotados;
...
function formatQuestOut(row) {
  return {
    ...
    bonecosPilotados: row.bonecos_pilotados || [],
