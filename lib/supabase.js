 ...
 */
export async function addQuest(payload) {
  const questId = genId();
  const questRow = {
    id: questId,
 ...
 */
export async function updateQuest(id, payload) {
  ...
  if (Array.isArray(payload.drops)) {
    ...
    for (const d of payload.drops) {
      ...
      toUpsert.push({
        ...
        char: d.char || '',
        dropador: d.dropador || '',
        sold_price: d.soldPrice !== undefined ? (d.soldPrice || '') : undefined,
        sold_date: d.soldDate !== undefined ? (d.soldDate || '') : undefined
      });
    }
...
export async function updateDropSale(dropId, payload) {
  const row = {};
...
export async function saveConfig(config) {
  // Se vier 'password' no payload, hasha antes de gravar.
  // (Idealmente esse caminho deveria deixar de aceitar password — vai sair
  // num commit seguinte que separa /api/change-password.)
  let passwordToWrite = config.password;
  if (passwordToWrite && !isBcryptHash(passwordToWrite)) {
    passwordToWrite = await hashPassword(passwordToWrite);
  }
